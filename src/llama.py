...
class LlamaRotaryEmbedding(torch.nn.Module):
    ...
    def forward(self, x, seq_len=None, offset=0):
        ...
        if seq_len > self.max_seq_len_cached:
            ...
            t = torch.arange(self.max_seq_len_cached, device=x.device, dtype=self.inv_freq.dtype) + offset
            freqs = torch.einsum("i,j->ij", t, self.inv_freq)
            # Different from paper, but it uses a different permutation in order to obtain the same calculation
...
class Attention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.rand_offset = config.off_llama
    ...
    def forward(
        ...
        if self.rand_offset:
            cos, sin = self.rotary_emb(value_states, seq_len=kv_seq_len, offset=torch.randint()) # may need to specify a max and a size component here
        else:
